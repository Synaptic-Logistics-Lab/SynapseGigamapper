    ...
    def set_sample_weights(self, args: argparse.ArgumentParser) -> None:
        ...
        if args.class_bal:
            ...
            for d in self.dataset:
                if args.class_bal_key in d and isinstance(d[args.class_bal_key], dict):
                    labels_dict = d[args.class_bal_key]
            ...
            for label, count in label_counts.items():
                # Calculate the class weight as the inverse of the class frequency
                weight_per_label[label] = (total_samples / float(count))

            print("Class counts are: {}".format(label_counts))
            print("Label weights are {}".format(weight_per_label))

            self.weights = []
            for d in self.dataset:
                if args.class_bal_key in d and isinstance(d[args.class_bal_key], dict):
                    labels_dict = d[args.class_bal_key]
                    # Calculate the sample weight as the maximum of its class weights
                    sample_weight = 0.0
                    positive_labels = [label for label, value in labels_dict.items() if value == 1]
                    if len(positive_labels) > 0:
                        for label in positive_labels:
                            if label in weight_per_label:
                                sample_weight += weight_per_label[label] / len(positive_labels)
                    self.weights.append(sample_weight)
                else:
                    # If the key does not exist, assign a default weight
